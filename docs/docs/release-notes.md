# 发布说明

## 版本 1.2.0 (当前)

**发布日期:** 2025 年 2 月

### 新功能
- 添加 SQL Server 视图支持 (`list_views`、`describe_view`、`get_view_definition`)
- 为数据修改操作添加 `confirm` 参数增强安全性
- 改进 SQL Server 可空字段检测逻辑

### 改进
- 完善 MCP 工具的 outputSchema 定义
- 更新工具描述和文档
- 优化错误消息的中文化

### 错误修复
- 修复 SQL Server `IS_NULLABLE` 字段映射错误
- 修复 `write_query` 工具的 TRUNCATE 支持验证

## 版本 1.1.0

**发布日期:** 2025 年 1 月

### 新功能
- 添加 MySQL 数据库支持(读/写/查询、架构等)
- 支持通过 CLI 和配置传递 MySQL 端口
- 支持 MySQL 的 AWS IAM 认证

### 改进
- 改进 MySQL 的端口验证和调试日志
- 更新 MySQL 和端口使用的文档和示例

## 版本 1.0.2

**发布日期:** 2025 年 4 月

### 新功能
- 使用新的 Docusaurus 实现完全重写文档
- 使用蓝色配色方案更新品牌
- 添加全面的连接参考指南
- 改进数据库工具文档
- 新的快速入门指南,便于入门

### 改进
- 更好的数据库连接错误处理
- 增强 SQL 查询性能,特别是对于大结果集
- 将 Node.js 依赖项更新到最新的稳定版本
- 改进凭据处理的安全性
- 为敏感连接信息添加环境变量支持

### 错误修复
- 修复使用 Windows 身份验证时 SQL Server 的连接问题
- 更正 PostgreSQL RETURNING 子句处理中的错误
- 解决连接池中的内存泄漏问题
- 修复 SQLite 导出中的日期格式问题

## 版本 1.0.1

**发布日期:** 2025 年 4 月

### 新功能
- PostgreSQL 数据库的初步支持
- 添加 CSV 和 JSON 格式的导出功能
- 引入带有备忘录功能的业务洞察跟踪

### 改进
- 增强失败查询的错误报告
- 更好地处理大结果集
- 优化 SQL Server 的连接池

### 错误修复
- 修复密码中包含特殊字符的 SQL Server 身份验证
- 解决长时间运行的查询的超时问题
- 解决某些表名的架构检测问题

## 版本 1.0.0

**发布日期:** 2025 年 4 月

### 初始发布功能
- 支持 SQLite 和 SQL Server 数据库
- 基本 SQL 查询执行(SELECT、INSERT、UPDATE、DELETE)
- 表管理(CREATE、ALTER、DROP)
- 架构内省
- Claude Desktop 的 MCP 集成
- 基于 Node.js 的实现,支持跨平台
